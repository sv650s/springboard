...
class TestQuadl(unittest.TestCase):
    ...
    def test_sort_data(self):
        ...
        self.assertEquals(len(unsorted_list), 4, 
            f"list should be length of 4 but found {len(unsorted_list)}")

        # sort by index 3 should give us 04, 02, 03
        sorted_list = self.quadl.sort_data(unsorted_list, 3)
        self.assertEquals(sorted_list[0][0], '2017-01-04', 
            f'first should be 04. found {sorted_list[0][0]}')
        self.assertEquals(sorted_list[1][0], '2017-01-02', 
            f'second should be 02. found {sorted_list[1][0]}')
        self.assertEquals(sorted_list[2][0], '2017-01-03', 
            f'third should be 03. found {sorted_list[2][0]}')
        self.assertEquals(sorted_list[3][0], '2017-01-05', 
            f'forth should be 05. found {sorted_list[3][0]}')
    ...
    def test_calculate_median(self):
        ...
        self.assertEquals(median2, 2.50, f'median should be 2.50 but got {median2}')
    ...
    def test_calculate_stats(self):
        ...
        self.assertEquals(average_trading_volume, round(self.df['Traded Volume'].mean(), 2), \
        ...
        self.df = self.df.assign(two_day_change=self.df['Close'].rolling(
            window=2, center=False).apply(lambda x: round(x[1] - x[0], 2), raw=True))
